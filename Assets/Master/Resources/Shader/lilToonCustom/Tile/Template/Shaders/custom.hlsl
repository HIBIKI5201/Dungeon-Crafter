...
LIL_V2F_OUT.objectPos = (LIL_MATRIX_M[0].w,LIL_MATRIX_M[1].w,LIL_MATRIX_M[2].w);
...
// Inserting a process into pixel shader
    ...
    #define OVERRIDE_MAIN \
        float3 scale = (length(LIL_MATRIX_M[0].xyz),length(LIL_MATRIX_M[1].xyz),length(LIL_MATRIX_M[2].xyz));\
        float3 offsetPos = float3(\
            fd.positionWS.x + length(LIL_MATRIX_M[0].xyz) % 2 * (_TileSize / 2),\
            fd.positionWS.y + length(LIL_MATRIX_M[1].xyz) % 2 * (_TileSize / 2),\
            fd.positionWS.z + length(LIL_MATRIX_M[2].xyz) % 2 * (_TileSize / 2)\
            );\
        float3 positionWS = lilTransformOStoWS(LIL_MATRIX_I_M[3].xyz)*-1+ offsetPos;\
        float TileSize = 1/_TileSize;\
        float3 Node_UV = positionWS * TileSize;\
        float3 Node_Blend = pow(abs(normalize(input.normalWS)), 1);\
        Node_Blend /= dot(Node_Blend, 1.0);\
        float4 Node_X = LIL_SAMPLE_2D(_MainTex, sampler_MainTex, Node_UV.zy);\
        float4 Node_Y = LIL_SAMPLE_2D(_MainTex, sampler_MainTex, Node_UV.xz);\
        float4 Node_Z = LIL_SAMPLE_2D(_MainTex, sampler_MainTex, Node_UV.xy);\
        fd.col = Node_X*Node_Blend.x+ Node_Y*Node_Blend.y+Node_Z*Node_Blend.z;\
